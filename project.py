...
def gconnect():
  ...
  if result['user_id']!=gplus_id:
    response = make_response(json.dumps("Token's userid doesn't match given user id"), 401)
    response.headers['Content-Type']= 'application/json'
    return response
  if result['issued_to']!=CLIENT_ID:
    response = make_response(json.dumps("Token's client id doesn't match app's client id"), 401)
    response.headers['Content-Type']= 'application/json'
    print "Token's client id doesn't match app's client id"
    return response
...
def editRestaurant(restaurant_id):
  ...
  if editedRestaurant.user_id != login_session['user_id']:
    return "<script> function myfunction(){alert('You are not authorized to edit this restaurant. Please create your own restaurant to delete.')}</script><body onload='myfunction()'>"
...
def editMenuItem(restaurant_id, menu_id):
  ...
  if editedItem.user_id != login_session['user_id']:
    return "<script> function myfunction(){alert('You are not authorized to edit this item. Please create your own restaurant to delete.')}</script><body onload='myfunction()'>"
...
def deleteMenuItem(restaurant_id,menu_id):
  ...
  if itemToDelete.user_id != login_session['user_id']:
    return "<script> function myfunction(){alert('You are not authorized to delete this item. Please create your own restaurant to delete.')}</script><body onload='myfunction()'>"
